...
test_that(".familyTreeToAncestorTable returns correct results for new familyTree example", {
  familyTree <- tibble::tribble(
    ~parent_concept_id, ~child_concept_id,
    45557624, 2000504169,
    45596282, 45557624,
    ...
    45596282, 45557628,
    45596282, 45545188,
    2000302838, 2000302841,
    2000302838, 2000302842,
    2000302838, 2000302839,
    2000302838, 2000302840,
    2000302839, 2000403990,
    2000403990, 2000403991,
    2000403990, 2000403992,
    2000403990, 2000403999,
    2000403992, 2000403993,
    2000403992, 2000403994,
    2010010561, 2010010562,
    2010010561, 2010010563,
    2010010561, 45596282,
    2010010561, 45591560
  )

  # Test 1: Root = 2010010561
  ancestorTable <- .familyTreeToAncestorTable(familyTree, 201001635)
  # Spot checks for some known nodes and paths
  expect_true(any(ancestorTable$descendant_concept_id == 2010010561 & ancestorTable$levels == "0-0" & ancestorTable$paths == 1))
  expect_true(any(ancestorTable$descendant_concept_id == 45591560 & ancestorTable$levels == "1-1" & ancestorTable$paths == 1))
  expect_true(any(ancestorTable$descendant_concept_id == 45596282 & ancestorTable$levels == "1-1" & ancestorTable$paths == 1))

  # Should also reach leaf nodes
  expect_true(any(ancestorTable$descendant_concept_id == 45545188))
  expect_true(any(ancestorTable$descendant_concept_id == 45557625))
  expect_true(any(ancestorTable$descendant_concept_id == 45557628))

  # Test 2: Root = 45596282
  ancestorTable2 <- .familyTreeToAncestorTable(familyTree, 45596282)
  # First generation children
  expect_setequal(
    ancestorTable2$descendant_concept_id[ancestorTable2$levels == "1-1"],
    c(45557624, 45557625, 45557628, 45545188)
  )
  # Check that 45596282 is present as self
  expect_true(any(ancestorTable2$descendant_concept_id == 45596282 & ancestorTable2$levels == "0-0"))
  
  # Test 3: Leaf node only self
  ancestorTable3 <- .familyTreeToAncestorTable(familyTree, 45545188)
  expect_equal(
    ancestorTable3,
    tibble::tibble(
      descendant_concept_id = 45545188L,
      levels = "0-0",
      paths = 1L
    )
  )
